...
if (t.type === 'grass' || t.type === 'sapling' || t.type === 'berry') { t.ready = false; t.regrow = rand(60, 105); }
else t.dead = true;
G.stats.gather++;
G.chunkDirty = true;
sfx.pick();
...
if (t.hp <= 0) { t.dead = true; G.chunkDirty = true; if (t.type === 'wildhive') { dropItem(t.x, t.y, 'honey', 1 + irand(0, 2)); dropItem(t.x, t.y, 'stinger', 1 + irand(0, 2)); } else { dropItem(t.x, t.y, 'honey', 1); dropItem(t.x, t.y, 'stinger', 1); } addText(t.x, t.y - 30, 'Улей разрушен', '#ff8a76'); sfx.fall(); }
} else if (t.type === 'bee') {
