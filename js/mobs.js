...
if (held && held.id === 'captured_bee') {
if (t.beeCount >= 4) { msg('Улей уже полон (4/4 пчёл)'); break; }
t.beeCount++;
held.n--;
if (held.n <= 0) {
...
ent({ type: 'bee', x: t.x + rand(-20, 20), y: t.y + rand(-20, 20), hp: 10, hive: t, aggro: false, vx: 0, vy: 0, s: rand(100), stingT: 0, pollinateT: rand(5, 15) });
addText(t.x, t.y - 44, '🐝 Пчела заселена (' + t.beeCount + '/4)', '#e8b83a');
msg('Пчела в улье (' + t.beeCount + '/4)');
sfx.oink();
if (t.beeCount === 4) msg('🍯 Улей полностью заселён! Жди мёд.', 'hint');
break;
}
// СБОР МЁДА (нужна маска)
if (!G.equip.head || !ITEMS[G.equip.head.id] || !ITEMS[G.equip.head.id].beeProtect) {
if (t.beeCount >= 4) msg('🐝 Надень маску пчеловода, чтобы собирать мёд!');
else msg('🐝 Улей пустой. Поймай сачком 4 пчёл и засели их сюда.');
break;
}
if (t.honey > 0) {
const ha2 = Math.min(t.honey, 1 + Math.floor(Math.random() * 2));
t.honey -= ha2;
if (!canAdd('honey', ha2)) { msgFull(); break; }
addInv('honey', ha2);
addText(t.x, t.y - 34, '🍯 +' + ha2 + ' мёд', '#e8b83a');
sfx.pick();
} else msg('Мёда пока нет — пчёлы ещё не натаскали.');
} break;
case 'pig': {
held = getActiveItem();
...
if (t.item === 'comb' && !G.flags.combHint) { G.flags.combHint = 1; msg('🪹 Соты! Нужны для крафта улья.', 'hint'); }
if (t.item === 'captured_bee' && !G.flags.capBeeHint) { G.flags.capBeeHint = 1; msg('🐝 Пчела в банке! Подойди к пустому улью и засели её.', 'hint'); }
break;
case 'campfire': {
held = getActiveItem();
...
case 'bee': {
held = getActiveItem();
if (held && held.id === 'bugnet' && !t.aggro) {
// ЛОВЛЯ пчелы сачком
if (!canAdd('captured_bee', 1)) { msgFull(); break; }
addInv('captured_bee', 1);
t.dead = true; G.chunkDirty = true;
addText(t.x, t.y - 20, '🐝 Пчела поймана!', '#e8b83a');
sfx.pick();
burst(t.x, t.y - 4, 8, '#ffd75e', 60);
held.dur--;
if (held.dur <= 0) {
if (G.equip.hands && G.equip.hands.id === held.id) G.equip.hands = null;
else { for (let i = 0; i < 10; i++) if (G.inv[i] && G.inv[i].id === 'bugnet') { G.inv[i] = null; break; } }
msg('Сачок порвался!'); sfx.breakIt(); dirtyInv(); equipDirty = true;
} else {
if (G.equip.hands && G.equip.hands.id === held.id) G.equip.hands.dur = held.dur;
}
hotbarDirty = true; equipDirty = true;
if (!G.flags.bugnetHint) { G.flags.bugnetHint = 1; msg('🐝 Поймай ещё пчёл и отнеси их к пустому улью', 'hint'); }
} else if (held && held.id === 'bugnet' && t.aggro) {
msg('Пчела агрессивна! Подожди или убей её.');
cancelJob(); doSwing(t);
} else {
cancelJob(); doSwing(t);
}
} break;
case 'rabbit': case 'spider': case 'giant': case 'chess_horse': cancelJob(); doSwing(t); break;
case 'pond':
...
const hasBees = (e.beeCount || 0) > 0 || e.type === 'wildhive';
const maxBees = e.type === 'wildhive' ? 4 : (e.beeCount || 0);
if (beeCount < maxBees && !e.dead && !e.angry && hasBees) {
ent({ type: 'bee', x: e.x + rand(-30, 30), y: e.y + rand(-30, 30), hp: 10, hive: e, aggro: false, vx: 0, vy: 0, s: rand(100), stingT: 0, pollinateT: rand(3, 10) });
}
if (!e.dead && !e.angry) {
let bees2 = 0;
forEachInRadius(e.x, e.y, 150, o => { if (o.type === 'bee' && !o.dead && o.hive === e) bees2++; return false; });
if (bees2 >= 2 && hasBees) e.honey = Math.min(3, e.honey + 0.5);
}
}
...
e.pollinateT = (e.pollinateT || 0) - localDt;
const angry = e.aggro || (e.hive && e.hive.angry);
// ОПЫЛЕНИЕ: ищем цветок/куст/траву у улья
if (!angry && e.hive && !e.hive.dead && !e.pollTarget && e.pollinateT <= 0 && playing) {
if (Math.random() < localDt * 0.4) {
const candidates = [];
forEachInRadius(e.hive.x, e.hive.y, 380, o => {
if (o.dead) return false;
if (o.type === 'flower' || (o.type === 'berry' && o.ready !== false) || (o.type === 'grass' && o.ready !== false)) {
candidates.push(o);
}
return false;
});
if (candidates.length) e.pollTarget = pick(candidates);
}
}
let target = null;
if (angry && playing) {
target = { x: px, y: py };
} else if (e.pollTarget && !e.pollTarget.dead && !angry) {
// летим к цели опыления
const dxP = e.pollTarget.x - e.x, dyP = e.pollTarget.y - e.y;
const distP = Math.hypot(dxP, dyP);
if (distP > 10) {
e.vx += (dxP / distP * 70 - e.vx) * localDt * 4;
e.vy += (dyP / distP * 70 - e.vy) * localDt * 4;
} else {
e.vx *= Math.max(0, 1 - localDt * 4);
e.vy *= Math.max(0, 1 - localDt * 4);
if (!e.pollinating) { e.pollinating = true; e.pollinateDur = rand(3, 5); }
e.pollinateDur -= localDt;
if (e.pollTarget.ready === false && e.pollTarget.regrow !== undefined) {
e.pollTarget.regrow -= localDt * 2;
}
if (e.pollinateDur <= 0) {
e.pollinating = false;
e.pollTarget = null;
e.pollinateT = rand(12, 25);
if (Math.random() < 0.5) burst(e.x, e.y - 4, 3, '#ffd75e', 30, .4);
}
}
return updateBeePhysics(e, localDt, pd2, playing, angry, px, py);
} else if (e.hive && !e.hive.dead) {
target = { x: e.hive.x, y: e.hive.y };
} else {
target = { x: e.x + Math.sin(e.s + G.t * 0.3) * 40, y: e.y + Math.cos(e.s + G.t * 0.4) * 40 };
}
...
// общая физика и укусы пчелы (вынесено, чтобы не дублировать)
...
const dxB = (e.hive ? e.hive.x : e.x) - e.x, dyB = (e.hive ? e.hive.y : e.y) - e.y;
const distHive = Math.hypot(dxB, dyB);
if (e.hive && !e.hive.angry && distHive > 300) e.aggro = false;
